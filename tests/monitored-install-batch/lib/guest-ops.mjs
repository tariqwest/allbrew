...
export async function syncAllbrewSrcToVM(h, hostSrcPath, vmDest) {
  ...
  const script = [
    ...
    `  git -C "$SRC" fetch origin "refs/heads/$BRANCH:refs/remotes/origin/$BRANCH" --depth 1 2>&1 || git -C "$SRC" fetch origin "$BRANCH:refs/remotes/origin/$BRANCH" --depth 1 2>&1 || git -C "$SRC" fetch origin --depth 1 2>&1`,
    `  git -C "$SRC" checkout -B "$BRANCH" "refs/remotes/origin/$BRANCH" 2>&1 || git -C "$SRC" checkout -B "$BRANCH" "origin/$BRANCH" 2>&1 || git -C "$SRC" checkout "$BRANCH" 2>&1 || true`,
    `  git -C "$SRC" reset --hard "refs/remotes/origin/$BRANCH" 2>&1 || git -C "$SRC" reset --hard "origin/$BRANCH" 2>&1 || true`,
    `else`,
