...
const INDEX = join(BATCH, "state", process.env.BATCH_INDEX || "retry-rate-limit-index.jsonl");
const URLS_PATH = join(BATCH, "state", process.env.TH_TH_BATCH_URLS_FILE || "retry-rate-limit-urls.json");
...
async function guest(session, cmd, description, opts = {}) {
  ...
  const tokenExport = token
    ? `export GITHUB_TOKEN=${JSON.stringify(token)}\nexport GH_TOKEN=${JSON.stringify(token)}\n`
    : "";
  const fullCmd = tokenExport + cmd;
  try {
    const stdout = await runAsProjectUser(fullCmd, description, {
      session,
...
if [ -n "\${GITHUB_TOKEN:-}" ]; then
  /opt/homebrew/bin/allbrew config set-token "$GITHUB_TOKEN" >/dev/null
fi
...
# ensure tap configured (recover if lost)
if ! /opt/homebrew/bin/allbrew config show >/dev/null 2>&1; then
  mkdir -p "$HOME/homebrew-allbrew/Formula" "$HOME/homebrew-allbrew/Casks" "$HOME/.config/allbrew"
  /opt/homebrew/bin/allbrew config set-tap "$HOME/homebrew-allbrew" >/dev/null 2>&1 || true
fi
if [ -n "\${GITHUB_TOKEN:-}" ]; then
  /opt/homebrew/bin/allbrew config set-token "$GITHUB_TOKEN" >/dev/null 2>&1 || true
fi
...
TOKEN_ARGS=()
if [ -n "\${GITHUB_TOKEN:-}" ]; then TOKEN_ARGS+=(--token "$GITHUB_TOKEN"); fi
$AB "$URL" --name "$NAME" --verbose "\${TOKEN_ARGS[@]}" >"$LOG" 2>&1
EC=$?
...
try {
  ...
  writeFileSync(
    join(BATCH, "state", process.env.TH_BATCH_PROGRESS || "retry-rate-limit-progress.json"),
    JSON.stringify(
      {
  ...
  writeFileSync(join(BATCH, "summary", process.env.TH_BATCH_SUMMARY || "retry-rate-limit-summary.json"), JSON.stringify(summary, null, 2));
