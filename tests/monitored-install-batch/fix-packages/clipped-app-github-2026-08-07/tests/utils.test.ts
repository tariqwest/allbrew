...
describe("isAppAsset", () => {
  ...
  it("matches PascalCase product-style app zips without OS/arch tags", () => {
    expect(isAppAsset("Clipped.zip")).toBe(true);
    expect(isAppAsset("Clipped-1.7.2.zip")).toBe(true);
    expect(isAppAsset("MyApp.zip")).toBe(true);
    expect(isAppAsset("Raycast.zip")).toBe(true);
  });

  it("rejects lowercase/hyphen portable CLI zips as app assets", () => {
    expect(isAppAsset("license-plist.zip")).toBe(false);
    expect(isAppAsset("clipped.zip")).toBe(false);
    expect(isAppAsset("foo-bar.zip")).toBe(false);
  });

  it("rejects SwiftPM artifactbundle zips", () => {
