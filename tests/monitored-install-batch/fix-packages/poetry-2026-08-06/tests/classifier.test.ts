...
describe("classify", () => {
  ...
  describe("package registries", () => {
    ...
    it("matches PyPI JSON API URLs", () => {
      const result = classify("https://pypi.org/pypi/poetry/json");
      expect(result.type).toBe("pip-package");
      expect(result.packageName).toBe("poetry");
    });

    it("maps official Poetry installer host to pip-package poetry", () => {
      const result = classify("https://install.python-poetry.org");
      expect(result.type).toBe("pip-package");
      expect(result.packageName).toBe("poetry");
    });

    it("maps official Poetry installer host with trailing slash", () => {
      const result = classify("https://install.python-poetry.org/");
      expect(result.type).toBe("pip-package");
      expect(result.packageName).toBe("poetry");
    });
