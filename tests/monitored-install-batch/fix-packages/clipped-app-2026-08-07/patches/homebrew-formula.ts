...
export async function generateHomebrewFormula(name: string, options: any = {}) {
  ...
  let info: HomebrewApiInfo;
  try {
    info = (await fetchJson(apiUrl)) as HomebrewApiInfo;
  } catch (err: any) {
    const msg = String(err?.message || err);
    if (/\bHTTP 404\b/.test(msg)) {
      throw new Error(
        `Homebrew core formula "${token}" not found (API 404 for ${apiUrl}). ` +
          `formulae.brew.sh only lists homebrew/core; third-party taps (e.g. user/tap/formula) ` +
          `are not mirrored. Check the formula token or use the project's install URL instead.`,
      );
    }
    throw err;
  }
