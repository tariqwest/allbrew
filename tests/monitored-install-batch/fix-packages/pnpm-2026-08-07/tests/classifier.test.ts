...
describe("classify", () => {
  ...
  describe("known installer hosts", () => {
    it("maps get.pnpm.io install script to npm-package pnpm", () => {
      const result = classify("https://get.pnpm.io/install.sh");
      expect(result.type).toBe("npm-package");
      expect((result as { packageName?: string }).packageName).toBe("pnpm");
      expect((result as { via?: string }).via).toBe("known-installer-host");
    });
  });
