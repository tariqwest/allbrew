...
describe("napari GUI package maps", () => {
  it("maps napari to pyqt6 root extras", () => {
    expect(KNOWN_ROOT_EXTRAS.napari).toEqual(["pyqt6"]);
    expect(KNOWN_PYTHON_IMPORT_VERSION_TEST.napari).toBe("napari");
  });

  it("uses import-based testDoBody for napari", async () => {
    const napari = {
      info: {
        name: "napari",
        version: "0.8.0",
      ...
      urls: [
        {
          packagetype: "bdist_wheel",
          python_version: "py3",
          filename: "napari-0.8.0-py3-none-any.whl",
          url: "https://files.pythonhosted.org/packages/xx/napari-0.8.0-py3-none-any.whl",
          digests: { sha256: "aa".repeat(32) },
        },
      ],
    };
    const numpy = {
      info: { name: "numpy", version: "2.0.0", summary: "numpy", requires_dist: [] },
      urls: [
        {
          packagetype: "bdist_wheel",
          python_version: "py3",
    ...
    const qtpy = {
      info: { name: "qtpy", version: "2.4.0", summary: "qtpy", requires_dist: [] },
      urls: [
        {
          packagetype: "bdist_wheel",
          python_version: "py3",
          filename: "qtpy-2.4.0-py3-none-any.whl",
          url: "https://files.pythonhosted.org/packages/xx/qtpy-2.4.0-py3-none-any.whl",
          digests: { sha256: "cc".repeat(32) },
        },
      ],
    };
    const pyqt6 = {
      info: { name: "PyQt6", version: "6.7.0", summary: "PyQt6", requires_dist: [] },
      urls: [
        {
          packagetype: "bdist_wheel",
          python_version: "py3",
          filename: "PyQt6-6.7.0-py3-none-any.whl",
          url: "https://files.pythonhosted.org/packages/xx/PyQt6-6.7.0-py3-none-any.whl",
          digests: { sha256: "dd".repeat(32) },
        },
      ],
    };
    ...
    global.fetch = mock((url: string) => {
      const u = String(url);
      let body: unknown = {
        info: { name: "unknown", version: "1.0.0", summary: "x", requires_dist: [] },
        urls: [],
      };
      if (u.includes("/napari/")) body = napari;
      else if (u.includes("/numpy/")) body = numpy;
      else if (u.includes("/qtpy/")) body = qtpy;
      else if (u.toLowerCase().includes("/pyqt6/")) body = pyqt6;
      return Promise.resolve({ ok: true, json: () => Promise.resolve(body) });
    }) as any;

    const payload = await collectPipPackagePayload("napari");
    expect(payload.testBinName).toBe("napari");
    expect(payload.testDoBody).toContain("libexec}/bin/python -c");
    expect(payload.testDoBody).toContain("import napari");
    expect(payload.testDoBody).not.toContain("#{bin}/napari --version");
    // pyqt6 extra should pull PyQt6 resource
    expect(payload.resourcesBlock).toContain('resource "PyQt6"');
  });
});
