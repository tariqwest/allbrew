...
export async function matchOfficialCaskByHomepage(
  ...
  for (const token of tokens) {
    try {
      ...
      // Exact registrable domain match (chronoid.app → chronoid.app)
      ...
      // Alternate TLD with same product label (chronoid.com → chronoid.app)
      // when the hostname label equals the cask token (avoids random sites).
      const pageLabel = pageHost.split(".")[0] || "";
      const caskLabel = caskHost.split(".")[0] || "";
      const caskToken = info.token || token;
      if (
        pageLabel &&
        caskLabel &&
        pageLabel === caskLabel &&
