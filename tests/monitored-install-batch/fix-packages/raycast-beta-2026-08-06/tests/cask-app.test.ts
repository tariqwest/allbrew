...
describe("collectCaskAppPayload", () => {
  ...
  it("derives cask token from filename", async () => {
    ...
    // Filename: Seaquel_2026.4.8_aarch64.dmg → strip version suffix → "Seaquel" → toCaskToken
...
describe("raycast cask download helpers", () => {
  it("extracts version from Raycast_vX.Y.Z CDN path", async () => {
    const { extractVersionFromUrl, canonicalizeCaskDownloadUrl, filenameFromDownloadUrl } =
      await import("../../../lib/generators/cask-app.ts");
    const signed =
      "https://raycast-releases.example.r2.cloudflarestorage.com/Raycast_v1.104.24_hash_universal.dmg?X-Amz-Signature=abc&response-content-disposition=attachment%3B%20filename%3D%22Raycast.dmg%22";
    expect(extractVersionFromUrl(signed)).toBe("1.104.24");
    const canon = canonicalizeCaskDownloadUrl(signed);
    expect(canon.rewritten).toBe(true);
    expect(canon.version).toBe("1.104.24");
    expect(canon.url).toContain("releases.raycast.com/releases/1.104.24/download");
    expect(filenameFromDownloadUrl(signed)).toBe("Raycast.dmg");
  });

  it("collectCaskAppPayload rewrites signed Raycast URL and sets version", async () => {
    const payload = await collectCaskAppPayload(
      "https://cdn.example/Raycast_v1.104.24_abc_universal.dmg?X-Amz-Signature=deadbeef",
      { name: "raycast-beta", appName: "Raycast.app" },
    );
    expect(payload.versionLine).toContain("1.104.24");
    expect(payload.url).toContain("releases.raycast.com/releases/1.104.24/download");
    expect(payload.appOrPkgBlock).toContain("Raycast.app");
    expect(payload.homepageLine).toContain("raycast.com");
  });
});
