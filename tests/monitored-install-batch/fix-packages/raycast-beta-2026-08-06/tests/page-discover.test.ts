...
import {
  ...
  parseGithubRepoHome,
  seedVendorDownloadCandidates,
  demotePresignedCandidates,
...
describe("seedVendorDownloadCandidates", () => {
  it("seeds stable Raycast download for raycast.com homepage", () => {
    const c = seedVendorDownloadCandidates("https://www.raycast.com/");
    expect(c.length).toBe(1);
    expect(c[0].url).toBe("https://releases.raycast.com/download");
    expect(c[0].kind).toBe("cask-dmg");
    expect(c[0].score).toBeGreaterThanOrEqual(100);
  });

  it("demotes AWS presigned URLs", () => {
    const ranked = demotePresignedCandidates([
      {
        url: "https://cdn.example/app.dmg?X-Amz-Signature=abc",
        kind: "cask-dmg",
        score: 175,
        evidence: [],
        source: "webview",
    ...
    expect(ranked[0].url).toContain("releases.raycast.com");
    expect(ranked.find((x) => x.url.includes("X-Amz"))!.score).toBeLessThan(175);
  });
});
