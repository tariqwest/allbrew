...
export async function generateHomebrewCask(name: string, options: any = {}) {
  ...
  try {
    ...
    if (/HTTP 404/.test(msg)) {
      throw new Error(
        `No official Homebrew cask named "${token}" exists (API 404). ` +
          `Case C fail-closed: allbrew will not invent a package from the homebrew-cask monorepo. ` +
          `Try brew search, the product homepage URL, or a different token. Original: ${msg}`,
      );
    }
    throw err;
  }
