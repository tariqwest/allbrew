...
export function isDownloadHubPath(url: string): boolean {
  try {
    ...
    if (
      ...
      // TokenBar-class vendor APIs stream binaries (not marketing HTML)
      /(?:^|\/)api\/download(?:s)?(?:\/|$)/i.test(p)
    ) {
      return false;
    }
    return (
...
export function looksLikeExtensionlessArtifactUrl(url: string): boolean {
  try {
    ...
    if (
      ...
      // TokenBar-class SPAs: /api/download/latest → GitHub release DMG
      /(?:^|\/)api\/download(?:s)?(?:\/|$)/i.test(path) ||
...
/**
 * Same-site root-level "*.dmg" URLs reconstructed from SPA string literals that
 * only name the download file (Content-Disposition), not a real static path.
 * e.g. https://tokenbar.site/TokenBar-latest.dmg from R="TokenBar-latest.dmg".
 */
export function isLikelyScriptFilenamePlaceholderDmg(url: string, pageUrl: string): boolean {
  try {
    const u = new URL(url);
    if (!/\.dmg(?:\?|#|$)/i.test(u.pathname)) return false;
    if (!sameSite(url, pageUrl)) return false;
    const segs = u.pathname.split("/").filter(Boolean);
    // Bare filename at site root (or www root): /TokenBar-latest.dmg
...
export async function enrichDownloadHubPages(
  ...
  // Real install artifacts (or install scripts) short-circuit hub crawl.
  // mac-app-store / setapp alone do NOT — many SPAs still expose a better
  // direct DMG on /download that only lives in JS bundles.
  ...
  for (const hub of hubList) {
    try {
      ...
      if (opts.hubHtmlByUrl?.[hub]) {
        ...
        const fetched = await fetchTextLimited(hub, { maxBytes: 2_000_000, timeoutMs: 20_000 });
        body = fetched.body;
...
export async function enrichExtensionlessArtifactUrls(
  ...
  // SPA bundles often embed Content-Disposition filenames as root "/App-latest.dmg"
  // strings that 404, while the real binary is an extensionless API (TokenBar:
  // TokenBar-latest.dmg vs /api/download/latest). Still probe extensionless when
  // every same-site .dmg looks like a bare filename-at-root placeholder.
  const dmgCandidates = candidates.filter(
    (c) => c.kind === "cask-dmg" || /\.dmg(?:\?|#|$)/i.test(c.url),
  );
  const onlyFilenamePlaceholders =
    dmgCandidates.length > 0 &&
    dmgCandidates.every((c) => isLikelyScriptFilenamePlaceholderDmg(c.url, pageUrl));
  if (dmgCandidates.length > 0 && !onlyFilenamePlaceholders) return candidates;
  ...
  try {
    ...
    const guesses = [
      ...
      `${origin}/api/download/latest`,
      `${origin}/api/download/file`,
      `${origin}/api/downloads/latest`,
      `${origin}/download/mac`,
  ...
  for (const p of probes) {
    try {
      ...
      if (head.type === "cask-dmg") {
        // Beat unverified script-bundle filename DMGs (~190) so real API wins
        scored.score = Math.max(scored.score, onlyFilenamePlaceholders ? 200 : 120);
...
export async function discoverFromScriptBundles(
  ...
  for (const src of ordered.slice(0, scriptLimit)) {
    try {
      ...
      // Vendor download APIs: "/api/download/latest", "/api/download/file"
      const relApiDl =
        body.match(
          /["'`](\/api\/download(?:s)?(?:\/[^"'`\s)?#]*)?)["'`]/gi,
        ) || [];
      for (const raw of relApiDl) {
        const path = raw.replace(/^['"`]|['"`]$/g, "").trim();
        if (!path) continue;
        try {
          const abs = new URL(path, pageUrl).href;
          ...
          assertSafePublicFetchUrl(abs);
          candidates.push(
            scoreCandidateUrl(abs, pageUrl, ["script-bundle", "api-download-rel-path"]),
          );
        } catch {}
      }
  ...
  for (const u of preferred) {
    ...
    // Bare same-site "/Product-latest.dmg" from SPA filename strings often 404;
    // keep as weak hint only so HEAD-probed /api/download/* can win.
    if (isLikelyScriptFilenamePlaceholderDmg(u, pageUrl)) {
      scored.score = Math.min(scored.score, 60);
      scored.evidence.push("script-filename-placeholder-dmg");
    } else {
      scored.score += 35;
