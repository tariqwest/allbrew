...
describe("formulae.brew.sh official pages (popclip)", () => {
  test("classifies cask pages as homebrew-cask (popclip)", () => {
    const r = classify("https://formulae.brew.sh/cask/popclip");
    expect(r.type).toBe("homebrew-cask");
    expect(r.name).toBe("popclip");
  });
