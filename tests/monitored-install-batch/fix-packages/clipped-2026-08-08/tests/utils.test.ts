...
describe("isAppAsset", () => {
  ...
  it("matches versioned product zips without mac token (NetBar-style)", () => {
  ...
  it("matches bare PascalCase product zips (Clipped.zip)", () => {
    expect(isAppAsset("Clipped.zip")).toBe(true);
    expect(isAppAsset("MyApp.zip")).toBe(true);
  });

  it("rejects bare lowercase and source zips", () => {
