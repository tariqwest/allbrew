...
export function isAppAsset(assetName) {
  ...
  // or bare PascalCase product zips (e.g. Clipped.zip) are commonly single-platform
  // macOS .app distributions. cask-app-release still peeks inside for a real .app.
  if (!hasCpuArch) {
    if (isVersionedProductZipName(lower)) return true;
    if (isBareProductAppZipName(assetName, lower)) return true;
  }
  return false;
}
...
/** Clipped.zip / MyApp.zip — branded product name without platform tags. */
function isBareProductAppZipName(assetName: string, lowerName: string): boolean {
  if (!lowerName.endsWith(".zip")) return false;
  if (
    /(?:^|[^a-z])(?:src|source|sources|checksums?)(?:[^a-z]|$)/i.test(lowerName)
  ) {
  ...
  const baseOrig = assetName.slice(0, -".zip".length);
  // Product zips almost always use Title/PascalCase branding
  if (!/^[A-Z]/.test(baseOrig)) return false;
  if (!/^[A-Za-z][A-Za-z0-9._-]*$/.test(baseOrig)) return false;
  // Reject if name embeds platform/cpu tokens
  if (
    /(?:^|[^a-z])(?:linux|windows|win32|android|freebsd|openbsd|arm64|aarch64|amd64|x86_64|x64|i386|darwin|macos|osx)(?:[^a-z]|$)/i.test(
      lowerName,
    )
  ) {
    return false;
