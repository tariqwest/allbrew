...
describe("alternateTldUrls", () => {
  it("swaps brand.tld marketing hosts (.app → .com)", () => {
    const alts = alternateTldUrls("https://aimeflux.app/");
    expect(alts).toContain("https://aimeflux.com/");
    expect(alts.some((u) => u.includes("aimeflux.io"))).toBe(true);
    expect(alts.every((u) => !u.includes("aimeflux.app"))).toBe(true);
  });
  ...
  it("skips multi-label hosts (subdomains)", () => {
    expect(alternateTldUrls("https://download.aimeflux.com/x")).toEqual([]);
