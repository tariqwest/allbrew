...
export function packageHintFromInstallUrl(url: string): string | undefined {
  try {
    ...
    // SDKMAN bootstrap: get.sdkman.io / sdkman.io
    if (/(?:^|\.)sdkman\.io$/i.test(host)) {
      return "sdkman";
    }
    const first = host.split(".")[0];
    // Skip generic CDN first labels (get, www, download, install, sh)
    if (
      first &&
      !/^(raw|github|githubusercontent|sh|get|www|download|install|cdn|api)$/i.test(first)
    ) {
...
export function analyzeInstallScript(
  ...
  // Official SDKMAN bootstrap (get.sdkman.io): installs under $HOME/.sdkman (or SDKMAN_DIR),
  // patches shell rc files, requires Bash 4+, and does not honor Homebrew PREFIX/DESTDIR.
  // No homebrew/core formula exists; refuse Empty-installation install-script wrappers.
  const SDKMAN_URL_RE = /(?:^|\/\/)(?:get\.)?sdkman\.io(?:\/|$)/i;
  if (
    SDKMAN_URL_RE.test(url) ||
    (/SDKMAN_DIR/i.test(text) &&
      /sdkman-init\.sh/i.test(text) &&
      (/api\.sdkman\.io/i.test(text) || /sdkman_auto_answer/i.test(text)))
  ) {
    signals.push("sdkman-bootstrap");
    return {
      kind: "home-dir-installer",
      packageHint: "sdkman",
      signals,
      reason:
        "Official SDKMAN installer installs under $HOME/.sdkman (SDKMAN_DIR), patches " +
        "shell profiles, requires Bash 4+, and does not produce Cellar PREFIX binaries. " +
        "There is no homebrew/core formula for SDKMAN; install via the upstream script " +
        "outside allbrew rather than wrapping get.sdkman.io as install-script.",
    };
  }
