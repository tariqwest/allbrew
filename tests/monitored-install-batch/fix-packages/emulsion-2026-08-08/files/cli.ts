...
async function handleCargoPackage(classification, opts) {
  const crateName = opts.crateName || opts.package || classification.crateName;
  if (!crateName) {
    throw new Error(
      "crates.io crate name required (use --crate-name / --package or a crates.io URL)",
    );
  }
  return await generateWithConfirmation(
    "cargo-package",
    { crateName, repoInfo: null, release: null },
    opts,
  );
}
