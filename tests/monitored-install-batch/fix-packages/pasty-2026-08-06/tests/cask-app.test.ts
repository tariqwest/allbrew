...
describe("collectCaskAppPayload", () => {
  ...
  it("handles GitHub /latest/ URL via release tag or :latest", async () => {
    ...
    // Prefer concrete version from GitHub API; offline/mock may fall back to :latest
    expect(
  ...
  it("Perplexity DMG: no version in URL produces version :latest", async () => {
    const payload = await collectCaskAppPayload(
      "https://cdn.perplexity.ai/downloads/Perplexity.dmg",
      { name: "perplexity", appName: "Perplexity.app", homepage: "https://www.perplexity.ai" },
    );
...
it("Postman: version extracted from CDN URL path", async () => {
  ...
  it("Postman /latest/ URL: emits version :latest", async () => {
    const payload = await collectCaskAppPayload(
  ...
  it("OnyX DMG: no version in filename produces version :latest", async () => {
    const payload = await collectCaskAppPayload(
  ...
  it("IconChamp ZIP: no version in URL produces version :latest", async () => {
    const payload = await collectCaskAppPayload(
  ...
  it("ego lite DMG: no version in CDN URL produces version :latest", async () => {
    const payload = await collectCaskAppPayload(
  ...
  it("Pasty GitHub latest DMG: always has a version stanza", async () => {
    const payload = await collectCaskAppPayload(
      "https://github.com/JordanH22/pasty/releases/latest/download/Pasty.dmg",
      { name: "pasty", appName: "Pasty.app", homepage: "https://pasty.dev" },
    );
    expect(payload.template).toBe("cask_app");
    expect(payload.name).toBe("pasty");
    expect(payload.versionLine.trim().length).toBeGreaterThan(0);
    expect(payload.versionLine).not.toBe("");
    // Must not leave version empty (Homebrew nil latest? crash)
    expect(payload.versionLine).toMatch(/version (:latest|"[^"]+")/);
    expect(payload.displayName).toBe("Pasty");
    expect(payload.appOrPkgBlock).toContain("Pasty.app");
  });
});
