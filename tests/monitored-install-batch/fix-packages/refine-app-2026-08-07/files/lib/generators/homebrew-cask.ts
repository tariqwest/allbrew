...
export async function matchOfficialCaskByHomepage(
  ...
  for (const token of tokens) {
    try {
      ...
      // Cross-TLD brand match: only when the probed token is the brand label
      // so example.com + --name superwhisper still returns null.
      const caskLabel = hostSldLabel(caskHost);
      const brandTldFlex =
        Boolean(pageLabel) &&
        pageLabel === caskLabel &&
        (token === pageLabel || (info.token || token) === pageLabel);
