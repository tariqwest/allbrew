...
describe("isBinaryAsset", () => {
  ...
  it("rejects binary delta / bsdiff patch assets", () => {
    expect(isBinaryAsset("deno-aarch64-apple-darwin.from-2.9.4.bsdiff")).toBe(
      false,
    );
    expect(isBinaryAsset("tool-macos-arm64.delta")).toBe(false);
  });
