...
describe("matchOfficialCaskByHomepage", () => {
  ...
  it("does not adopt ia-markdown-dictionary for ia.net/writer (ia-writer product)", async () => {
    const m = await matchOfficialCaskByHomepage(
      "https://ia.net/writer",
      "ia-writer",
    );
    // No official ia-writer cask; multi-product vendor must not steal dictionary tip.
    expect(m).toBeNull();
  });

  it("matches ia.net/presenter path to official ia-presenter cask", async () => {
    const m = await matchOfficialCaskByHomepage(
      "https://ia.net/presenter",
      "ia-presenter",
    );
    expect(m).not.toBeNull();
    expect(m!.token).toBe("ia-presenter");
  });
