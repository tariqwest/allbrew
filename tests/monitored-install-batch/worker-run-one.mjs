#!/usr/bin/env bun
/**
 * Skill-aligned single-URL worker.
 *
 * Env:
 *   TH_BATCH_WORKER_ID, TH_BATCH_WORKER_USER, TH_BATCH_WORKER_TAP, TH_BATCH_WORKER_MOUNT
 *   TH_* harness overrides (set by orchestrator child env)
 *   TH_BATCH_ENTRY_JSON — JSON {idx,name,url,source}
 *   TH_BATCH_FIX_MODE=docs|off (default docs)
 *   TH_BATCH_STRICT_VERIFY=1
 *   GITHUB_TOKEN (optional)
 *
 * Prints one JSON outcome line to stdout prefixed with OUTCOME_JSON=
 */
...
let entry = null;
if (process.env.TH_BATCH_ENTRY_JSON) {
  try { entry = JSON.parse(process.env.TH_BATCH_ENTRY_JSON); } catch {}
}
if (!entry) {
  const urlArg = getArg("--url");
  if (urlArg) {
    entry = {
      idx: Number(getArg("--idx") || 0),
      url: urlArg,
      name: getArg("--name") || "pkg",
      endpoint: getArg("--endpoint") || undefined,
    };
  }
}

if (!entry?.url) {
  console.error("TH_BATCH_ENTRY_JSON or --url <url> argument required");
  process.exit(2);
}
...
async function main() {
  const endpointOverride = entry.endpoint || getArg("--endpoint") || process.env.TH_VM_ENDPOINT || "";
  let poolLease = null;
  if (endpointOverride) {
    ...
    const { acquirePoolSlot } = await import("./lib/vm-pool.mjs");
    poolLease = await acquirePoolSlot(name);
  }

  log(`start idx=${idx} ${name} ${url} endpoint=${poolLease?.endpoint?.id || 'default'}`);
  ...
  try {
    ...
    // Fetch formula ruby for service analysis
    ...
    // Strict verify when install exit 0
    if (exitCode === 0) {
      const v = await guest(
        h.runAsProjectUser,
        session,
        strictVerifyCmd({
          pkg,
          mountPoint,
          expectService: judgment.expected.service,
        }),
        `verify-${pkg}`,
        { timeout: 180_000 },
      );
      writeFileSync(join(runDir, "verify.txt"), v.stdout || "");
      verify = parseVerifyOutput(
        v.stdout || "",
        pkg,
        judgment.expected.service === true,
      );
    }

    const serviceMismatch = svcCmp.mismatch;
    if (serviceMismatch && exitCode === 0) {
      ...
      // map rate limit to blocked
      if (failureClass === "github_rate_limit") {
        status = "blocked";
      } else {
        status = "failed";
      }
    ...
    // Always uninstall for batch hygiene
    ...
    // Option A fix capture on failure
    if (status !== "success" && fixMode === "docs") {
      ...
      // Enrich outcome.fix before finalize
      const priorOutcome = {
        schemaVersion: 1,
        status,
        failureClass,
      ...
      writeFileSync(
        ...
        JSON.stringify(priorOutcome, null, 2) + "\n",
      );
    }
...
- service expected/observed: ${judgment.expected.service}/${judgment.codebaseObserved.serviceDetected}
- durationMs: ${Date.now() - started}
- fix-package: ${fixPackagePath || "(none)"}
- allbrew: ${allbrewVersion}

## Notes
Skill-aligned batch worker (option A fix capture; no auto-release).
`,
    );

    // Map failure classes finalize accepts
    const finalizeClass = [
      "generate_fail",
      "brew_fail",
      "service_mismatch",
    ...
    finalizeRunRecordHost({
      ...
      allbrewVersionFinal: allbrewVersion.split("\n").pop(),
    });

    // Re-open outcome to stamp batch fields
    try {
    ...
    writeFileSync(
      join(BATCH_LOGS, `${batchRunId}.outcome.json`),
      JSON.stringify(batchOutcome, null, 2),
    );
    ...
    if (poolLease) {
      try {
        const { releaseEndpointMutex } = await import("./lib/vm-pool.mjs");
        releaseEndpointMutex(poolLease);
      } catch (e) {
        log("pool lease release failed", e);
      }
    }
  }
}
