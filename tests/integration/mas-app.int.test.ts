...
describe.concurrent("mas-app integration", () => {
  ...
  it("xcode: extracts app id and produces payload", async () => {
    ...
    const payload = await collectMasAppPayload(url, { name: "xcode" });
  ...
  it("invalid URL: throws on missing app id", async () => {
    await expect(
      collectMasAppPayload("https://apps.apple.com/us/app/invalid/no-id", {
        name: "invalid",
      }),
    ).rejects.toThrow();
  });
