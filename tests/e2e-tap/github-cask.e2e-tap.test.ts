...
import { caskPath, remoteHasFile, getLatestCommitMessage, getWorkHeadSha, getRemoteHeadSha } from "./helpers/tap.ts";
import { existsSync } from "node:fs";

describe.skipIf(!E2E_TAP)("github-cask: cask-app-release", () => {
  let ctx: TestContext;
  const app = getFixtureApp("fake-cask-release");
  ...
  describe("update", () => {
    it("should detect new version via livecheck, regenerate, push, and upgrade", () => {
      ...
      mutateApp(ctx, "fake-cask-release", "2.0.0");
