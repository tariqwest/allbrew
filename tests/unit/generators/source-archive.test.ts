...
import { collectSourceArchivePayload } from "../../../lib/generators/source-archive.ts";
...
describe("collectSourceArchivePayload", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("returns payload with correct template identifier", async () => {
    const payload = await collectSourceArchivePayload(archiveInfo);
    expect(payload.template).toBe("source_archive");
  });
  ...
  it("detects meson build system", async () => {
    ...
    const payload = await collectSourceArchivePayload(mesonArchive);
  ...
  it("uses forcedBuildSystem when provided", async () => {
    ...
    const payload = await collectSourceArchivePayload(forced);
    expect(payload.installBody).toContain("meson");
  });
});
