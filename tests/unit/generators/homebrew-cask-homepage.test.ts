...
describe("matchOfficialCaskByHomepage", () => {
  ...
  it("does not adopt thingsmacsandboxhelper for Things product homepage + things-3 slug", async () => {
    const m = await matchOfficialCaskByHomepage(
  ...
  it("still matches helper when preferred slug names the helper cask", async () => {
    const m = await matchOfficialCaskByHomepage(
      "https://culturedcode.com/things",
