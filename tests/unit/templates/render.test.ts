...
describe("renderFormula", () => {
  ...
  it("renders install_script template", () => {
    ...
    const actual = renderFormula(payload);
    expect(actual).toContain("class Foo < Formula");
    expect(actual).toContain('ENV["FORCE"] = "1"');
    expect(actual).toContain('script = buildpath/"allbrew-install.sh"');
    expect(actual).toContain("content.gsub!");
    expect(actual).toContain("File.write(script, content)");
    expect(actual).toContain('system "bash", script.to_s, "-f"');
    expect(actual).toContain("if dir == bin");
    expect(actual).toContain('assert_path_exists bin/"foo"');
    expect(actual).toContain('assert_predicate bin/"foo", :executable?');
  });
