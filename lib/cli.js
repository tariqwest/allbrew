...
async function handleGithubRepoManual(url, opts) {
  ...
  switch (strategy) {
    ...
    case 'go-package': {
      ...
      return await generateWithConfirmation('go-package', { repoInfo, release, goModule }, opts);
    }
  ...
  if (!readme) {
    ...
    if (method) {
      ...
      switch (method.method) {
        ...
        case 'go':
          return await generateWithConfirmation('go-package', {
            repoInfo,
            release,
  ...
  if (buildSystem) {
    ...
    switch (buildSystem.method) {
      ...
      case 'cargo':
        return await generateWithConfirmation('cargo-package', { repoInfo, release }, opts);
      case 'go':
        return await generateWithConfirmation('go-package', { repoInfo, release }, opts);
      case 'build':
        return await generateWithConfirmation('build-from-source', {
          repoInfo,
          release,
...
async function generateWithConfirmation(generatorName, params, opts) {
  ...
  switch (generatorName) {
    ...
    case 'go-package': {
      ...
      result = await generateGoPackage(params.repoInfo, params.release, { ...mergedOpts, goModule: params.goModule });
      break;
    }
    case 'script-install': {
  ...
  if (result.type === 'formula') {
    ...
    console.log(chalk.dim(`  Install with: brew install --cask ${result.name}`));
  }
