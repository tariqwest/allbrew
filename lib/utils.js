...
export async function writeFormula(name, content) {
  const dir = join(PROJECT_ROOT, 'Formula');
...
export async function writeCask(name, content) {
  const dir = join(PROJECT_ROOT, 'Casks');
  await mkdir(dir, { recursive: true });
  const filePath = join(dir, `${name}.rb`);
  await writeFile(filePath, content, 'utf-8');
  return filePath;
