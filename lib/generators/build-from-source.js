...
export async function generateBuildFromSource(repoInfo, release, buildSystem, options = {}) {
...
function getDependencies(system) {
  switch (system) {
    ...
    case 'autotools':
      return ['"autoconf" => :build', '"automake" => :build', '"pkg-config" => :build'];
    case 'meson':
      return ['"meson" => :build', '"ninja" => :build', '"pkg-config" => :build'];
    case 'go':
      return ['"go" => :build'];
    default:
      return [];
