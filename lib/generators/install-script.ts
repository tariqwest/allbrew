...
export async function resolveInstallScriptVersion(
  ...
  // curl|bash install pages often embed github.com/owner/repo or release URLs.
  try {
    ...
    if (res.ok) {
      ...
      const gh = body.match(/https?:\/\/github\.com\/([A-Za-z0-9_.-]+)\/([A-Za-z0-9_.-]+)/);
      if (gh) {
        const owner = gh[1];
      ...
      const verInBody = body.match(/\b(?:VERSION|version)\s*=\s*["']v?(\d+\.\d+\.\d+[^"']*)["']/);
