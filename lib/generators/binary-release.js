import { toFormulaName, toClassName, extractVersionFromTag, matchAssetToArch, rubyString, guessLicenseIdentifier, writeFormula } from '../utils.js';
...
export async function generateBinaryRelease(repoInfo, release, options = {}) {
  ...
  const urlTemplate = (url, tag) => {
    return url.replace(version, '#{version}').replace(release.tagName, 'v#{version}');
  };
