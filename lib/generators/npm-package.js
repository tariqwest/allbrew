...
export async function generateNpmPackage(packageName, repoInfo = null, options = {}) {
  ...
  if (!latestVersion) throw new Error(`No latest version found for ${packageName}`);
  ...
  const desc = options.desc || pkgData.description || repoInfo?.description || `Install ${packageName}`;
  const homepage = pkgData.homepage || repoInfo?.homepage || `https://www.npmjs.com/package/${packageName}`;
