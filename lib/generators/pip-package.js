import { toFormulaName, toClassName, rubyString, guessLicenseIdentifier, writeFormula } from '../utils.js';

export async function generatePipPackage(packageName, repoInfo = null, options = {}) {
  const pypiData = await fetchPypiData(packageName);
  const latestVersion = pypiData.info.version;
  const sdist = pypiData.urls.find(u => u.packagetype === 'sdist') || pypiData.urls[0];

  if (!sdist) throw new Error(`No source distribution found for ${packageName} on PyPI`);

  const deps = await resolveTransitiveDeps(packageName, new Set());

  const name = options.name || toFormulaName(packageName);
  const className = toClassName(name);
  const desc = options.desc || pypiData.info.summary || repoInfo?.description || `Install ${packageName}`;
  const homepage = pypiData.info.home_page || pypiData.info.project_url || repoInfo?.homepage || `https://pypi.org/project/${packageName}/`;
...
async function fetchPypiData(packageName) {
  ...
  if (!response.ok) throw new Error(`PyPI lookup failed for ${packageName}: ${response.status}`);
  return response.json();
}

async function resolveTransitiveDeps(packageName, visited, maxDepth = 3, depth = 0) {
  ...
  try {
    ...
    for (const req of requires) {
      ...
      try {
        ...
        const sdist = depData.urls.find(u => u.packagetype === 'sdist') || depData.urls[0];
        if (sdist) {
          resources.push({
            name: depName,
            url: sdist.url,
        ...
        const transitive = await resolveTransitiveDeps(depName, visited, maxDepth, depth + 1);
